use textdistance_port::algorithms::edit::needleman_wunsch::{
    EqualityScorer, MatrixScorer, NeedlemanWunsch,
};
...
fn matches_identity_alignment_fixtures() {
    let algorithm = NeedlemanWunsch::with_scorer(1.0, EqualityScorer::new(1.0, -1.0));
...
fn matches_gap_five_and_matrix_fixtures() {
    let gap_five = NeedlemanWunsch::with_scorer(5.0, EqualityScorer::new(1.0, -1.0));
    ...
    let matrix_algorithm = NeedlemanWunsch::with_scorer(5.0, matrix);
    let prepared = text_pair("AGACTAGTTAC", "CGAGACGT", QValue::Elements);
    assert_eq!(matrix_algorithm.similarity(&prepared), 16.0);
}

#[test]
fn preserves_global_empty_unicode_qgram_and_normalized_behavior() {
    let algorithm = NeedlemanWunsch::new();

    let empty = text_pair("", "abc", QValue::Elements);
    assert_eq!(algorithm.similarity(&empty), -3.0);
    ...
    assert_eq!(algorithm.maximum(&different), 4.0);
    assert_eq!(algorithm.normalized_distance(&different), 0.625);
    assert_eq!(algorithm.normalized_similarity(&different), 0.375);
}
