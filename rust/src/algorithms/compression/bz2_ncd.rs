//! BZ2 NCD (Normalized Compression Distance).
//!
//! Source: `textdistance/algorithms/compression_based.py::BZ2NCD`, via
//! `_BinaryNCDBase` and `_NCDBase`. Full behavior card:
//! `docs/behavior-cards/manasa/bz2-ncd.md`. Dependency:
...
/// Source: `codecs.encode(data, 'bz2_codec')[15:]` strips the bz2 stream
/// header before measuring length. Python's default bz2 compresslevel is
/// 9, matched here by `Compression::best()`.
const HEADER_STRIP: usize = 15;
...
fn to_bytes(elements: &[Element]) -> Vec<u8> {
    ...
    for element in elements {
        match element {
            ...
            other => panic!("BZ2 NCD only supports character or byte sequences, got {other:?}"),
        }
    }
    bytes
}
...
/// Real compressors are not provably order-invariant the way the pure-math
/// NCD variants (Sqrt/Entropy) are, so unlike those, permutations here are
/// actually enumerated rather than skipped.
...
/// BZ2 NCD. See `docs/behavior-cards/manasa/bz2-ncd.md`.
#[derive(Default)]
pub struct Bz2Ncd;
